...
['1:30',9,'Uppvärmning','Lätt och ledigt'],
['1:30',12,'Tempo','Hitta rytmen'],
['1:00',14,'Backe – sittande','Öka motstånd'],
['1:00',11,'Återhämtning','Släpp av'],
['1:00',16,'Intervall','Kontrollerat hårt'],
['1:00',12,'Återhämtning','Lugnt'],
...
let passes=JSON.parse(stored||'null')||[demo,shortDemo], active=null, elapsed=0, running=false, timer=null;
...
function list(){stop();setBrand('MINA PASS','VÄLJ ETT PASS ELLER SKAPA ETT NYTT');setHomeButton(false);app.innerHTML=`<div class="toprow"><div><h1>Mina pass</h1><div class="muted">Välj ett pass eller skapa ett nytt</div></div><button class="primary" onclick="edit()">+ SKAPA NYTT PASS</button></div><div class="cards">${passes.map((p,i)=>`<div class="card"><h2>${p.name}</h2><div class="muted">${fmt(total(p))} · ${p.parts.length} delar</div>${bars(p)}<div class="actions"><button class="primary" onclick="run(${i})">▶ KÖR PASSET</button><button onclick="edit(${i})">REDIGERA</button><button onclick="duplicate(${i})">DUPLICERA</button><button onclick="del(${i})">RADERA</button></div></div>`).join('')}</div>`}
function edit(i){stop();setBrand('REDIGERA PASS','SKAPA ELLER ÄNDRA ETT PASS');setHomeButton(true);let p=i==null?{id:Date.now(),name:'Nytt pass',parts:[{id:1,time:'5:00',borg:10,moment:'Uppvärmning',instruction:''}]}:structuredClone(passes[i]);active={p,i};drawEdit()}
...
function run(i){startPlayer(passes[i])} function startPlayer(p){stop();setBrand(p.name,'');setHomeButton(true);active={p};elapsed=0;drawLive()}
